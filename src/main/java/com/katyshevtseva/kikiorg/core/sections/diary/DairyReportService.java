...
public class DairyReportService {
    ...
    public List<List<ReportCell>> getQuickReport() {
        return getReport(diaryService.getIndicators(),
                new Period(shiftDate(new Date(), DateUtils.TimeUnit.DAY, -7), new Date()));
    }
    ...
    private ReportCell convertToCell(Indicator indicator, Date date) {
        IndMark mark = diaryService.getMarkOrNull(indicator, date).orElse(null);
        if (mark == null) {
            return ReportCell.empty();
        }
        return ReportCell.filled(mark.getValueAndComment(), Styler.StandardColor.PASTEL_PINK, 200);
    }
    ...
    private List<ReportCell> getReportHead(List<Indicator> indicators) {
        ...
        for (Indicator indicator : indicators) {
            result.add(ReportCell.columnHead(indicator.getTitle()));
        }
        return result;
    }
}
